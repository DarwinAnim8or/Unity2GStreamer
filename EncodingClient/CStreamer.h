...
{
public:
    CStreamer(SOCKET aClient, bool useTCP);
    ~CStreamer();
...
private:
    ...
    void    SendRtpJpegPacket(const unsigned char * Jpeg, int JpegLen, int width, int height, const unsigned char * Tables, int TablesLen, int offset, bool End, int Chn);
